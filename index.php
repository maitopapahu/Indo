...
$botToken = "7529913637:AAFr-E6m5HRQLwhCRGUZBhT9pUfzcwRnG4Q";
    ...
    if($chat_id > 0){
      ...
      if($text == "/start"){
              ...
              // Set the keyboard array
                  ...
                  $keyboard = array(
                      array('Buy Indo Ig' , 'Buy Fresh' , 'Buy Add Fund Ig'),
                      array('My Balance', ),
                      array('Add Funds'),
...
🔥WELCOME TO INDO IG/FRESH 
 IG SELLER BOT 

🥲ANY PROBLEM? CONTACT @NITISH9589
 
YOU ARE A OLD USER</b>";
        sendMessageH($chat_id , $txt);

          } else {
              // data is not in the database
              sendMessageH($chat_id , "<b>HEY @$username</b>
              
<b>❤️HEMLLO

WELCOME TO INDO IG/FRESH IG 
SELLER BOT 

🥲ANY PROBLEM? CONTACT 
@NITISH9589
 
YOU ARE A BRAND NEW USER</b>");
 ...
 // Create the keyboard markup
     ...
     if($text == "Contact"){
         sendMessage($chat_id , "You Can DM [@Nitish9589] For Any Help & Support");
     }
...
CONTACT  @Nitish9589 TO ADD FUNDS MANUALLY</b>");
      }
      }
...
<b><i>Add Funds Ig</i> ► $available_fresh</b>

<b><i>INDO IG</i>   $available</b>

<b><i>PP</i>     $available_pp</b>
");
...
if($text == "Buy Add Funds IG"){
            $api_url = "https://api.telegram.org/bot$token/";
            $price = file_get_contents('admin/fresh.txt');;
            sendMessageH($chat_id , "<b>OLD IG :- 

PRICE PER OLD IG :- $price

HOW MANY DO YOU WANT?</b>");
            
              $keyboard = [
                ['Pre Paid 1 ', 'Pre Paid 2' ,'Pre Paid 3' , 'Pre Paid 4' , 'Pre Paid 5'],
                ['Pre Paid 6', 'Pre Paid 7' ,' Pre Paid 8' , 'Pre Paid 9' , 'Pre Paid 10'],
                ['Pre Paid 11', 'Pre Paid 12' ,'Pre Paid 13' , 'Pre Paid 14' , 'Pre Paid 15'],
                ['Pre Paid 16', 'Pre Paid 17' ,'Pre Paid 18' , 'Pre Paid 19' , 'Pre Paid 20'],
                ['Close']
            ];
            $reply_markup = json_encode([
                'keyboard' => $keyboard,
...
if($text == "Buy Add Fund Ig"){
            $api_url = "https://api.telegram.org/bot$token/";
            $price = file_get_contents('admin/pp.txt');;
            sendMessageH($chat_id , "<b>BEST IG CC 
NO REPLACEMENT</b>

<b>PRICE IS : $price Rs EACH

HOW MANY YOU WANTED TO BUY?</b>");
              $keyboard = [
                ['PREPAID 1 ', 'PREPAID 2' ,'PREPAID 3' , 'PREPAID 4' , 'PREPAID 5'],
                ['PREPAID 6', 'PREPAID 7' , 'PREPAID 8' , 'PREPAID 9' , 'PREPAID 10'],
                ['PREPAID 11', 'PREPAID 12' ,'PREPAID 13' , 'PREPAID 14' , 'PREPAID 15'],
                ['PREPAID 16', 'PREPAID 17' ,'PREPAID 18' , 'PREPAID 19' , 'PREPAID 20'],
                ['Close']
            ];
            $reply_markup = json_encode([
                'keyboard' => $keyboard,
        ...
        $quant = $part[1];
            ...
            $sql = "SELECT `funds` FROM `id` WHERE `chat_id` = '$chat_id'";
              ...
              if($funds >= $total){
                  ...
                  file_get_contents("https://api.telegram.org/bot$token/sendMessage?chat_id=$chat_id&text=INSUFFICIENT FUNDS To Add Funds Contact @phishing_php or Type /funds To Check Your Current Balance");
...
FAMPAY USERS PAY AND SEND SCREEN SHOT IN DM @NITISH9589</i></b>");
}
